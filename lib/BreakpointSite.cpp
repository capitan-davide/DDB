...
void DDB::BreakpointSite::enable() {
  ...
  // We are saving the first 8 bits here because we are gonna patch them with an
  // 'int3' instruction next.
...
void DDB::BreakpointSite::disable() {
  ...
  // Restore the original data, removing the 'int3' istructions that was patched
  // in when this breakpoint was enabled.
